...
function buildTransaction(inputs_from_api, fee, senders_address, send_amount, receipient_address, public_key, arbitraryData = []) {
  var fee = (new BigNumber(fee).times('1000000000000000000000000000')).toFixed()
  // init empty tx
  var transaction = { siacoinInputs: new Array(), siacoinOutputs: new Array(), minerFees: new Array(), transactionSignatures: new Array() }
  // get sum of input values
  ...
  if (arbitraryData.length > 0) {
    transaction.arbitraryData = arbitraryData;
  }
...
function buildBatchTransaction(inputs_from_api, fee, senders_address, send_amounts, receipient_addresses, public_key, arbitraryData = []) {
  var fee = (new BigNumber(fee).times('1000000000000000000000000000')).toFixed();
  // init empty tx
  var transaction = { siacoinInputs: new Array(), siacoinOutputs: new Array(), minerFees: new Array(), transactionSignatures: new Array() }
  // get sum of input values
...
function sigHash(txn, parentID, keyIndex, timelock) {
  let tohash = Buffer.concat([
    ...
    encodeInt(txn.arbitraryData.length)
  ]);

  if (txn.arbitraryData.length > 0) {
    tohash = Buffer.concat([tohash, ...txn.arbitraryData.map(encodeArbitraryData)]);
  }

  tohash = Buffer.concat([
    tohash,
    // signature metadata
  ...
  return blake2b(32).update(tohash).digest()
...
function transactionID(txn) {
  let tohash = Buffer.concat([
    encodeInt(txn.siacoinInputs.length),
    ...txn.siacoinInputs.map(encodeInput2),
    encodeInt(txn.siacoinOutputs.length),
    ...txn.siacoinOutputs.map(encodeOutput),
    encodeInt(0), // fileContracts
    encodeInt(0), // fileContractRevisions
...
function encodeArbitraryData(ad) {
  // 将 Base64 字符串转换为 Buffer
  const dataBuffer = Buffer.from(ad, 'base64');

  // 获取数据的长度
  const dataLength = dataBuffer.length;

  // 将长度 Buffer 和数据 Buffer 拼接成一个新的 Buffer
  const resultBuffer = Buffer.concat([encodeInt(dataLength), dataBuffer]);
